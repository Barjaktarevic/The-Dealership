        ...
        <Link to={`/manufacturers/${make.abbreviation}`} onClick={action(() => CarsStore.getAllCarsByManufacturer(make.abbreviation))}>
            <div key={make.id} className="relative group flex space-x-4 items-center border-4 border-cyan-700 rounded-lg shadow-lg shadow-slate-500 h-96 p-1 ,md:p-2 hover:scale-95 transition duration-100 cursor-pointer">
                ...
                <div className='w-3/5 text-left'>
                    <h1 className='text-sm md:text-lg uppercase text-cyan-300'>Abbreviated to: <span className='text-xs md:text-base text-slate-50 capitalize'>{make.abbreviation}</span></h1>
                    <h1 className='text-sm md:text-lg uppercase text-cyan-300'>Founded in: <span className='text-xs  md:text-base text-slate-50 capitalize'> {make.founded} </span></h1>
                    <h1 className='text-sm md:text-lg uppercase text-cyan-300'>Headquarters: <span className='text-xs  md:text-base text-slate-50 capitalize'> {make.headquarters}</span></h1>
                    <p className='text-sm md:text-lg uppercase text-cyan-300'>Description:  <span className='text-xs  md:text-base text-slate-50 capitalize'> {make.description}</span></p>
                </div>
